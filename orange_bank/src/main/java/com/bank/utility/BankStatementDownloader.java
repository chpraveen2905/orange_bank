...
public class BankStatementDownloader {
	...
	public BankStatementDownloader(List<Transaction> transactions, String startDate, String enddate) {
		super();
	...
	private void writeTableData(PdfPTable table) {
		for (Transaction transaction : transactions) {
			...
			table.addCell(transaction.getDestinationAccount() != null ? transaction.getDestinationAccount().getNumber() : "--" );
	...
	public void export(HttpServletResponse response) throws DocumentException, IOException {
		...
		pN.add("Account No.: " + transactions.get(0).getSourceAccount().getNumber());
		document.add(pN);
		...
		pA.add("Ifsc Code: " + transactions.get(0).getSourceAccount().getIfscCode());
		document.add(pA);
